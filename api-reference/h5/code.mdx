...
<body>
  ...
  <script>
    // The duix.js used for demonstration may not be the latest. Install the latest SDK via npm
    const duix = new window.Duix()
    function init() {
      const sign = document.querySelector('#sign').value // Server-generated signature
      const conversationId = document.querySelector('#conversationId').value // Duix platform conversation ID
      ...
      duix.on('intialSucccess', () => {
        console.info('initialSuccess')
        // At this point, the initialization is successful and start can be called
        // Some browsers (especially mobile) require a user gesture. It’s recommended to call start via a user click. Even if muted, some browsers won’t play without a click
