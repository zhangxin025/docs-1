...
| sign | string   |   Yes    |  Authentication string. [How to get sign?](../api/openAPI) |
...
> **Note:** This method needs to be called after the `initialSuccess` event is triggered. The `initialSuccess` event indicates that all resources are prepared. As follows:
>
> ```js
...
#### speakAudioStream(option: Object): Promise

Drive digital humans to speak, supporting the use of streaming audio for driving.

```javascript
duix.speakAudioStream({audio: 'https://your.website.com/xxx'})
```
...
| Name| Type |Required | Description | 
| :-----------------| :-------------- | :----------------------------------------------------------- | ------    | 
| audio | string   |   No    |  The streaming audio address for the digital human's response.|
| interrupt | boolean   |   No    |  Whether to interrupt the previous speech |
