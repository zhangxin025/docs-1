...
When integrating with the duix-openapi-v2 platform, generate tokens in your own backend. Refer to the token guide:

[How to get token](./getToken)

## Management Interfaces

Management endpoints require a token in the request header.
...
Query real-time concurrency for an app.
...
List active (in-call) sessions for an app.
...
Other session platforms can be integrated as follows. Through your digital human, the Duix platform will POST to your defined remote URL to obtain answers.

<img src="../assets/_tmp_dingtalkgov_qt_clipbord_pic_5.png" alt="image.png"/>

Example: Refer to this open-source demo for a simple dialog integration.

Request: The Duix platform will send question requests (the user’s question) via POST to your remote URL in the format below.
...
Query conversation details by Duix conversation ID.
